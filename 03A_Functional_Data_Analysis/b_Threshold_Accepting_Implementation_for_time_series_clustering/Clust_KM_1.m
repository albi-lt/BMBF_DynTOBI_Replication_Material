function [best,best1,objf_mean,objf_var,Dbest] = Clust_TA_1(m,n,runs,nn,un,iter,x);
...
filer = ['results\Clust_KM_1_',mat2str(m),'_',mat2str(n),'_',mat2str(iter),'.data'];
...
%     ***** verbose = 3 ***********************************************
if (vmod >= 3)
    f17 = fopen('results\Clust_KM_1.log','w');
    vtext='log file open            ';
...
[cidx, ctrs, sumd] = kmeans(x, m, 'Distance','sqeuclidean','MaxIter',iter, ...
                    'Replicates',runs, 'OnlinePhase','on','Options',opts);
...
%     
    ...
    vtext='Clust_KM_1 finished sucessful';
    displv(vmod,1,vtext,nvval);
